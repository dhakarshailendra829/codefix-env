...
class CodeFixAction(BaseModel):
    ...
    line_number:  Optional[int]   = Field(None, ge=1, description="Target line (1-indexed, for edit/insert/delete)")
    new_content:  Optional[str]   = Field(None, description="New line content (for edit/insert)")
    reasoning:    Optional[str]   = Field(None, description="Agent's chain-of-thought (logged, not used by env)")
