    ...
    def step(self, action: CodeFixAction):
        ...
        try:
            if action.action_type == "run_tests":
                ...
                feedback = f"Passed {passed}/{len(self._state.test_cases)} tests. Score: {score:.2f}"
                reward = score * 0.3
            elif action.action_type == "edit_line" and action.line_number is not None and action.new_line is not None:
    ...
    def _run_tests(self):
        try:
            ...
            for i, test in enumerate(self._state.test_cases):
                try:
                    inp = test["input"]
                    result = safe_globals[func_name](*inp) if isinstance(inp, (list, tuple)) else safe_globals[func_name](inp)
